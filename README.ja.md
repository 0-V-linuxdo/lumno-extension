...
その後、`chrome://extensions/` でデベロッパーモードを有効のままにし、リポジトリのルートをパッケージ化されていない拡張機能として読み込みます。開発版は Chrome プロファイルの起動時に一度自動更新されるため、Chrome を閉じている間に生成したアセットを起動後に手動で再読み込みする必要はありません。Chrome の起動中にバックグラウンドスクリプトまたは `manifest.json` を変更した場合は、引き続きすぐに手動で再読み込みしてください。
