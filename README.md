...
<p align="center">Firefox / Zen port: <code>0.9.51-firefox-v1.6.0</code></p>
...
1. Download [`lumno-0.9.51-firefox-v1.6.0.zip`](https://github.com/0-V-linuxdo/lumno-extension/releases/tag/0.9.51-firefox-v1.6.0) and unzip it.
...
`npm run package:store` reads the version from `manifest.json` and creates `dist/lumno-store-v<version>.zip`. It requires `zip` and `zipinfo` to be available on the system. `npm run package:firefox` writes `dist/lumno-0.9.51-firefox-v1.6.0.zip`.
