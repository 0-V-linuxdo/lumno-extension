...
- 自定义壁纸库保存在本地 IndexedDB，云端只保存当前生效的浅色/深色壁纸（最多两张）；自定义快捷方式图标保存在本地扩展存储并全部备份。上传对象都进入用户私有 Storage 目录。
- 插件在跳转网页登录前一次性披露同步和统计范围；用户确认并完成登录后，同时启用账号同步和每日聚合使用统计，不再提供独立统计开关。
- Access Token、Refresh Token 只放扩展源私有的 IndexedDB；设备 ID、自动处理记录和待发队列放 `chrome.storage.local`，均不进入浏览器同步。
...
- 当前生效的浅色/深色壁纸主图和缩略图（最多两组）
- 用户主动导入的快捷方式自定义图标
- 原始显示名称、MIME、尺寸、字节数、SHA-256

上传前，壁纸源文件最多 25 MiB，并在客户端重新编码为 WebP（云端主图最多 2 MiB、缩略图最多 160 KiB）；快捷方式图标源文件最多 10 MiB，并重新渲染为 128×128 PNG（最多 96 KiB）。原始 EXIF、文件名内嵌内容和非图片尾随数据不会直接进入云端。单用户云端最多保存两张当前生效壁纸和 20 个快捷方式图标，活跃媒体合计最多 10 MiB。对象路径由服务端生成、以用户 ID 开头，Bucket 不公开。

按全部文件都达到上限计算：`2 × (2 MiB + 160 KiB) + 20 × 96 KiB = 6.1875 MiB`。10 MiB 服务端配额为替换过程和边界情况留出余量；配额按服务端收到的真实字节计算，不采用客户端声明值。

客户端限制只负责体验，不能作为安全边界。`media-asset` Edge Function 会根据实际二进制重新检查 PNG/WebP 签名、文件结构、尺寸、元数据、尾随数据和字节数，并重新计算 SHA-256；只有通过检查的图片才以 Service Role 写入 Storage 和 metadata。服务不做常态化图片内容审核，也不把扩展名或客户端 MIME 当成可信判断。

媒体资源另有 40 次/账号/小时、32 MiB/账号/UTC 日、256 MiB/账号/月的上传预算，以及 128 MiB/账号/月的下载预算。项目全部活跃媒体达到 900 MiB 时停止新增，以免耗尽托管存储余量。认证客户端没有 Storage 写入、删除或直接下载权限，也不能修改 `lumno_assets`；因此自定义客户端无法通过伪造 metadata、先写后删或任意路径把 Bucket 当网盘。

导入壁纸只写入本地库，不会立即上传。用户连续试选时，每次选择都会重置 30 秒稳定计时；关闭壁纸面板时立即提交最终选择。手动同步、登录后首次同步和 15 分钟周期同步是恢复兜底。上传新槽位前先删除不再生效的云端壁纸，但不会删除任一设备的本地壁纸库。
...
1. 版本一致：原子更新并将版本加一。
2. 版本不一致：服务端不覆盖，客户端从 Outbox 摘除旧操作以终止重试死循环；客户端自动采用服务端版本并清理本地冲突记录。
...
冲突默认采用云端优先策略：共享的服务端版本不会被本机静默覆盖；如果冲突记录产生后本机已经基于最新服务端版本产生了新的修改，则保留这条新修改并继续上传。标量配置不做字段级魔法合并，数组或对象也不会在不了解业务语义时自动拼接。
...
3. 先拉取账号远端版本，再通过版本化 Outbox 上传本机新增或自动策略保留的值。
4. 当前生效的浅色/深色自定义壁纸从本机 IndexedDB 单独上传到私有 Storage，不塞进配置 JSON；本地库中的其他壁纸不上传。
...
- 设置变更后约 1 秒尝试同步；
- 壁纸选择变化后等待 30 秒稳定窗口，关闭壁纸面板时立即同步最终选择；普通 1 秒配置同步不上传壁纸字节；
...
- 配置采用“云端已变化时不静默覆盖”，冲突按配置键自动选择云端版本；失败操作不会永远重试。
...
- 退出后产生的本机改动继续写入游客模式工作副本。重新登录同一个账号时恢复原设备 ID 和版本基线，先拉取服务器全量：服务器版本没变的本机离线改动沿用原 `base_version` 上传；服务器版本已前进的键自动采用云端版本，不按登录先后静默覆盖。登录另一个账号时不会复用原账号检查点。
...
| 非扩展用途或违法图片 | 私有且不可分享；只接受规范化图片；每账号两个壁纸槽位、20 个图标、10 MiB 活跃额度和上传/下载预算；900 MiB 全局停写闸门 | 当前不做常态内容审核；收到有效举报或法律通知时按个案处理。若未来增加公开分享、社区或任意文件能力，必须在上线前重新引入内容治理评审 |
...
| 多设备冲突体验 | 等值冲突自动消解；真实冲突默认采用云端版本，并保留基于最新版本产生的新本机修改 | 继续把快捷方式等结构化列表从整组冲突细化为按稳定 ID 合并 |
...
- `src/shared/cloud-sync-state.js`：Outbox、版本和冲突检测纯逻辑。
