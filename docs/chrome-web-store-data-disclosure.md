...
> Lumno 是一个本地优先的浏览器效率工具，提供命令栏、新标签页、搜索与标签页操作，并在用户主动登录后提供 Lumno 配置、壁纸和快捷方式自定义图标的跨设备同步。

用户在插件内确认开启账号同步时，界面会同时披露产品使用统计；统计只用于理解 Lumno 功能采用情况和改进产品，不用于广告、用户画像、信用评估或出售数据。
...
| User activity / 用户活动 | 是 | 仅在用户确认开启账号同步并完成登录后，上传白名单功能次数以及配置的布尔值、枚举和数量。 |
...
- 除提供服务所需的 Supabase、Google 和 GitHub 外，不向第三方披露；私有同步图片不发送给第三方内容审核服务；
- 遵守 Chrome Web Store User Data Policy 的 Limited Use 要求；
- 传输使用 HTTPS，账号业务表与内部保留期表强制 RLS，用户媒体 Bucket 为私有；
- 用户可退出登录以停止同步和统计，或永久删除账号和云端数据。

## 数据保留与公开链接

- 账号和同步数据：保留到用户删除相应数据或永久删除账号；
- 云端只保存当前生效的浅色/深色壁纸（最多两张）以及正在使用的快捷方式自定义图标；被替换或删除的云端媒体会从活动存储及 metadata 中删除；
...
- Supabase 与 Lumno Web 的 host permissions：登录、同步私有配置和媒体、随账号同步启用的统计以及账号删除。
